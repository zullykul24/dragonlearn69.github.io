...
var true_answer = 0;// Nam test
var right_balls = 0; // count the right balls
...
/*function check_1(x)    // Check the first answer
{
    ...
    {  //Wrong answer
        ...
        message_1.style.color = "red";
        if(check_subPoint == 0 && right_balls != 0)subPoint();
        check_subPoint++;
    }
}
...
function commit() {  //When u give 3 true answers, the Next button appears 
    ...
    var cap = document.getElementById("caption");
    var line = document.getElementsByClassName("line");
    if (true_answer == 3 && lesson < numberOfLessons - 1) { //Change to - 1 when finish
    ...
    if (lesson == (numberOfLessons - 1) && true_answer == 3) { // Finish all questions of lessons // Change to -1 
...
function finish_screen() {
    ...
    question_1.innerHTML = "Your score: " + right_balls;
    question_1.style = " font-size: 40px; text-align: center; line-height: 50px; width: 960px";

    if (right_balls == 5) question_1.innerHTML += "<br>Perfect!";
